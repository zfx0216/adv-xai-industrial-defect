"""
The main function of this code file is to generate an accuracy evaluation image of the current explanatory method
...
Step two, you need to set 'output_folder_path_increase' to the absolute path of the folder where the generated evaluation images for improving classification values are saved
Step three, you need to set the absolute path of the folder where the generated evaluation images for reducing classification values will be saved to 'output_folder_path_decrease'
Step four, you need to set "json path" as the absolute path of the index file, which is located in a file named "output class indicators. json" in the upper level folder
Step five, you need to set "weights_path" as the absolute path of the weight file
"""
...
def main():
    ...
    for file_name in file_list:
        ...
        for i in range(224):
            for j in range(224):
                # The following criteria correspond to 8 evaluation metrics, and you can choose to generate corresponding evaluation images
                if grayscale_cam[i][j] >= 0.5:
                    ...
                    if actual_image_transform_matrix_R[i][j] < 0:
                        actual_image_R_channel_increase[i][j] = actual_image_channel_R[i][j] * ratio_decrease
                        actual_image_R_channel_decrease[i][j] = actual_image_channel_R[i][j] * ratio_increase
                    ...
                    if actual_image_transform_matrix_G[i][j] < 0:
                        actual_image_G_channel_increase[i][j] = actual_image_channel_G[i][j] * ratio_decrease
                        actual_image_G_channel_decrease[i][j] = actual_image_channel_G[i][j] * ratio_increase
                    ...
                    if actual_image_transform_matrix_B[i][j] < 0:
                        actual_image_B_channel_increase[i][j] = actual_image_channel_B[i][j] * ratio_decrease
                        actual_image_B_channel_decrease[i][j] = actual_image_channel_B[i][j] * ratio_increase
